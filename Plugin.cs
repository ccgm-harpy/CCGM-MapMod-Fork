...
namespace MapMod
{
    ...
    public class Plugin : BasePlugin
    {
        public static void Setup(){
            ...
            defaultModes = gamemodeManager.allPlayableGameModes;
            updateIndex().Wait();
            ...
            setupMaps();
            registerMaps();
            string serverShortName = ccgmGetRequest("server_short_name");
            ccgmPostJson("server_status", "{\""+ serverShortName + "\": " + "{\"status\": \"ReadyForInject\"}}");
        }
        public static string ccgmPostJson(string path, string json)
        {
            var url = $"http://127.0.0.1:5555/{path}";
        ...
        public static string ccgmGetRequest(string path)
        {
            var url = $"http://127.0.0.1:5555/{path}";
        ...
        {
            return ccgmGetRequest("mode_string");
        }
        public static string getMapString()
        {
            return ccgmGetRequest("map_string");
        }
        public static bool isMapModServer()
        {
            return ccgmGetRequest("is_mapmod_server") == "True";
        }
        public static void setupModes()
        {
            int modeNum = 0;
            var modeList = new Il2CppSystem.Collections.Generic.List<GameModeData>();
            string modeString = getModeString();
            foreach (GameModeData mode in defaultModes)
            {
                if (modeString[modeNum] == '1')
                {
                    modeList.Add(mode);
                }
                modeNum++;
            }
            gamemodeManager.allPlayableGameModes = modeList;
        }
        public static void setupMaps()
        {
            int mapNum = 0;
            var mapList = new Il2CppSystem.Collections.Generic.List<Map>();
            string mapString = getMapString();
            foreach (Map m in defaultMaps)
            {
                if (mapString[mapNum] == '1')
                {
                    mapList.Add(m);
                }
                mapNum++;
            }
            mapManager.playableMaps = mapList;
        }
        public static void registerMaps(){
            var mapList = new System.Collections.Generic.List<Map>(defaultMaps);
            int mapnum = 62;
            bool mapModServer = isMapModServer();
            foreach (Map m in customMaps){
                ...
                if (System.IO.File.Exists(mapfolder+"\\map.config")){
                    ...
                    if (config.Contains("lobbymap") && mapModServer)
                    {
                ...
                if (selectCustomMaps.Value && mapModServer)
                {
        ...
        public static Il2CppSystem.Collections.Generic.List<GameModeData> defaultModes;
